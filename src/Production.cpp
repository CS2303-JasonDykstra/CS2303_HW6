...
bool Production::production(int argc, char* argv[]){
	 ...
	 */
	b.welcomeScreen ();
	b.systemMessage ("                            Hit <ENTER> to continue!\n");
	b.clearScreen();
	...
	switch (option) {
	case 1: b.manuallyPlaceShipsOnGameBoard (playerOneGameBoard, ship);
	break;
	case 2: b.randomlyPlaceShipsOnGameBoard (playerOneGameBoard, ship);
	break;
	}
	 ...
	 */
	b.randomlyPlaceShipsOnGameBoard (playerTwoGameBoard, ship);
		...
		case PLAYER_TWO:
			...
			if (shot == 1) {
				...
				if (cardinals[NORTH] == TRUE) {  /* NORTH */
					...
					b.checkBoundsOfCardinal (cardinals, north, NORTH);
					targetTemp = target;
					targetTemp.row = north;
				...
				if (cardinals[SOUTH] == TRUE) {  /* SOUTH */
					...
					b.checkBoundsOfCardinal (cardinals, south, SOUTH);
					targetTemp = target;
					targetTemp.row = south;
					if (b.checkShot (playerOneGameBoard, targetTemp) == -1)
				...
				if (cardinals[WEST] == TRUE) {   /* WEST */
					...
					b.checkBoundsOfCardinal (cardinals, west, WEST);
					targetTemp = target;
					targetTemp.column = west;
					if (b.checkShot (playerOneGameBoard, targetTemp) == -1)
				...
				if (cardinals[EAST] == TRUE) {   /* EAST */
					...
					b.checkBoundsOfCardinal (cardinals, east, EAST);
					targetTemp = target;
					targetTemp.column = east;
					if (b.checkShot (playerOneGameBoard, targetTemp) == -1)
